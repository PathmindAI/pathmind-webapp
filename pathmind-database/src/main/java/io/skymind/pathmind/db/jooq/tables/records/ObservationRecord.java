/*
 * This file is generated by jOOQ.
 */
package io.skymind.pathmind.db.jooq.tables.records;


import io.skymind.pathmind.db.jooq.tables.Observation;

import javax.annotation.processing.Generated;

import org.jooq.Field;
import org.jooq.Record1;
import org.jooq.Record11;
import org.jooq.Row11;
import org.jooq.impl.UpdatableRecordImpl;


/**
 * This class is generated by jOOQ.
 */
@Generated(
    value = {
        "http://www.jooq.org",
        "jOOQ version:3.12.4"
    },
    comments = "This class is generated by jOOQ"
)
@SuppressWarnings({ "all", "unchecked", "rawtypes" })
public class ObservationRecord extends UpdatableRecordImpl<ObservationRecord> implements Record11<Long, Long, Integer, String, String, String, String, Double, Double, Double, Double> {

    private static final long serialVersionUID = -1595030295;

    /**
     * Setter for <code>public.observation.id</code>.
     */
    public void setId(Long value) {
        set(0, value);
    }

    /**
     * Getter for <code>public.observation.id</code>.
     */
    public Long getId() {
        return (Long) get(0);
    }

    /**
     * Setter for <code>public.observation.model_id</code>.
     */
    public void setModelId(Long value) {
        set(1, value);
    }

    /**
     * Getter for <code>public.observation.model_id</code>.
     */
    public Long getModelId() {
        return (Long) get(1);
    }

    /**
     * Setter for <code>public.observation.array_index</code>.
     */
    public void setArrayIndex(Integer value) {
        set(2, value);
    }

    /**
     * Getter for <code>public.observation.array_index</code>.
     */
    public Integer getArrayIndex() {
        return (Integer) get(2);
    }

    /**
     * Setter for <code>public.observation.variable</code>.
     */
    public void setVariable(String value) {
        set(3, value);
    }

    /**
     * Getter for <code>public.observation.variable</code>.
     */
    public String getVariable() {
        return (String) get(3);
    }

    /**
     * Setter for <code>public.observation.data_type</code>.
     */
    public void setDataType(String value) {
        set(4, value);
    }

    /**
     * Getter for <code>public.observation.data_type</code>.
     */
    public String getDataType() {
        return (String) get(4);
    }

    /**
     * Setter for <code>public.observation.description</code>.
     */
    public void setDescription(String value) {
        set(5, value);
    }

    /**
     * Getter for <code>public.observation.description</code>.
     */
    public String getDescription() {
        return (String) get(5);
    }

    /**
     * Setter for <code>public.observation.example</code>.
     */
    public void setExample(String value) {
        set(6, value);
    }

    /**
     * Getter for <code>public.observation.example</code>.
     */
    public String getExample() {
        return (String) get(6);
    }

    /**
     * Setter for <code>public.observation.min</code>.
     */
    public void setMin(Double value) {
        set(7, value);
    }

    /**
     * Getter for <code>public.observation.min</code>.
     */
    public Double getMin() {
        return (Double) get(7);
    }

    /**
     * Setter for <code>public.observation.max</code>.
     */
    public void setMax(Double value) {
        set(8, value);
    }

    /**
     * Getter for <code>public.observation.max</code>.
     */
    public Double getMax() {
        return (Double) get(8);
    }

    /**
     * Setter for <code>public.observation.min_items</code>.
     */
    public void setMinItems(Double value) {
        set(9, value);
    }

    /**
     * Getter for <code>public.observation.min_items</code>.
     */
    public Double getMinItems() {
        return (Double) get(9);
    }

    /**
     * Setter for <code>public.observation.max_items</code>.
     */
    public void setMaxItems(Double value) {
        set(10, value);
    }

    /**
     * Getter for <code>public.observation.max_items</code>.
     */
    public Double getMaxItems() {
        return (Double) get(10);
    }

    // -------------------------------------------------------------------------
    // Primary key information
    // -------------------------------------------------------------------------

    @Override
    public Record1<Long> key() {
        return (Record1) super.key();
    }

    // -------------------------------------------------------------------------
    // Record11 type implementation
    // -------------------------------------------------------------------------

    @Override
    public Row11<Long, Long, Integer, String, String, String, String, Double, Double, Double, Double> fieldsRow() {
        return (Row11) super.fieldsRow();
    }

    @Override
    public Row11<Long, Long, Integer, String, String, String, String, Double, Double, Double, Double> valuesRow() {
        return (Row11) super.valuesRow();
    }

    @Override
    public Field<Long> field1() {
        return Observation.OBSERVATION.ID;
    }

    @Override
    public Field<Long> field2() {
        return Observation.OBSERVATION.MODEL_ID;
    }

    @Override
    public Field<Integer> field3() {
        return Observation.OBSERVATION.ARRAY_INDEX;
    }

    @Override
    public Field<String> field4() {
        return Observation.OBSERVATION.VARIABLE;
    }

    @Override
    public Field<String> field5() {
        return Observation.OBSERVATION.DATA_TYPE;
    }

    @Override
    public Field<String> field6() {
        return Observation.OBSERVATION.DESCRIPTION;
    }

    @Override
    public Field<String> field7() {
        return Observation.OBSERVATION.EXAMPLE;
    }

    @Override
    public Field<Double> field8() {
        return Observation.OBSERVATION.MIN;
    }

    @Override
    public Field<Double> field9() {
        return Observation.OBSERVATION.MAX;
    }

    @Override
    public Field<Double> field10() {
        return Observation.OBSERVATION.MIN_ITEMS;
    }

    @Override
    public Field<Double> field11() {
        return Observation.OBSERVATION.MAX_ITEMS;
    }

    @Override
    public Long component1() {
        return getId();
    }

    @Override
    public Long component2() {
        return getModelId();
    }

    @Override
    public Integer component3() {
        return getArrayIndex();
    }

    @Override
    public String component4() {
        return getVariable();
    }

    @Override
    public String component5() {
        return getDataType();
    }

    @Override
    public String component6() {
        return getDescription();
    }

    @Override
    public String component7() {
        return getExample();
    }

    @Override
    public Double component8() {
        return getMin();
    }

    @Override
    public Double component9() {
        return getMax();
    }

    @Override
    public Double component10() {
        return getMinItems();
    }

    @Override
    public Double component11() {
        return getMaxItems();
    }

    @Override
    public Long value1() {
        return getId();
    }

    @Override
    public Long value2() {
        return getModelId();
    }

    @Override
    public Integer value3() {
        return getArrayIndex();
    }

    @Override
    public String value4() {
        return getVariable();
    }

    @Override
    public String value5() {
        return getDataType();
    }

    @Override
    public String value6() {
        return getDescription();
    }

    @Override
    public String value7() {
        return getExample();
    }

    @Override
    public Double value8() {
        return getMin();
    }

    @Override
    public Double value9() {
        return getMax();
    }

    @Override
    public Double value10() {
        return getMinItems();
    }

    @Override
    public Double value11() {
        return getMaxItems();
    }

    @Override
    public ObservationRecord value1(Long value) {
        setId(value);
        return this;
    }

    @Override
    public ObservationRecord value2(Long value) {
        setModelId(value);
        return this;
    }

    @Override
    public ObservationRecord value3(Integer value) {
        setArrayIndex(value);
        return this;
    }

    @Override
    public ObservationRecord value4(String value) {
        setVariable(value);
        return this;
    }

    @Override
    public ObservationRecord value5(String value) {
        setDataType(value);
        return this;
    }

    @Override
    public ObservationRecord value6(String value) {
        setDescription(value);
        return this;
    }

    @Override
    public ObservationRecord value7(String value) {
        setExample(value);
        return this;
    }

    @Override
    public ObservationRecord value8(Double value) {
        setMin(value);
        return this;
    }

    @Override
    public ObservationRecord value9(Double value) {
        setMax(value);
        return this;
    }

    @Override
    public ObservationRecord value10(Double value) {
        setMinItems(value);
        return this;
    }

    @Override
    public ObservationRecord value11(Double value) {
        setMaxItems(value);
        return this;
    }

    @Override
    public ObservationRecord values(Long value1, Long value2, Integer value3, String value4, String value5, String value6, String value7, Double value8, Double value9, Double value10, Double value11) {
        value1(value1);
        value2(value2);
        value3(value3);
        value4(value4);
        value5(value5);
        value6(value6);
        value7(value7);
        value8(value8);
        value9(value9);
        value10(value10);
        value11(value11);
        return this;
    }

    // -------------------------------------------------------------------------
    // Constructors
    // -------------------------------------------------------------------------

    /**
     * Create a detached ObservationRecord
     */
    public ObservationRecord() {
        super(Observation.OBSERVATION);
    }

    /**
     * Create a detached, initialised ObservationRecord
     */
    public ObservationRecord(Long id, Long modelId, Integer arrayIndex, String variable, String dataType, String description, String example, Double min, Double max, Double minItems, Double maxItems) {
        super(Observation.OBSERVATION);

        set(0, id);
        set(1, modelId);
        set(2, arrayIndex);
        set(3, variable);
        set(4, dataType);
        set(5, description);
        set(6, example);
        set(7, min);
        set(8, max);
        set(9, minItems);
        set(10, maxItems);
    }
}
