...
public class SearchRepository {
    ...
    protected static List<SearchResult> search(DSLContext ctx, long userId, String keyword) {
        return ctx
                .selectFrom(generateSearchQueryTable(ctx, userId, keyword))
    ...
    private static Table<Record6<String, Long, String, LocalDateTime, LocalDateTime, String>> generateSearchQueryTable(DSLContext ctx, long userId, String keyword){
        ...
        return ctx.select(DSL.inline(SearchResultItemType.PROJECT.getName()).as("itemType"), PROJECT.ID, PROJECT.NAME, PROJECT.DATE_CREATED, PROJECT.LAST_ACTIVITY_DATE, PROJECT.USER_NOTES)
                .from(PROJECT)
        ...
        .union(ctx.select(DSL.inline(SearchResultItemType.MODEL.getName()).as("itemType"), MODEL.ID, MODEL.NAME, MODEL.DATE_CREATED, MODEL.LAST_ACTIVITY_DATE, MODEL.USER_NOTES)
                .from(MODEL)
        ...
        .union(ctx.select(DSL.inline(SearchResultItemType.EXPERIMENT.getName()).as("itemType"), EXPERIMENT.ID, EXPERIMENT.NAME, EXPERIMENT.DATE_CREATED, EXPERIMENT.LAST_ACTIVITY_DATE, EXPERIMENT.USER_NOTES)
                .from(EXPERIMENT)
