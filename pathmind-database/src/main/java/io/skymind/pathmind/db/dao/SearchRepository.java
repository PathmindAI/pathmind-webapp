...
public class SearchRepository {
    ...
    protected static List<SearchResult> findSearchResults(DSLContext ctx, long userId, String keyword, int offset, int limit) {
        Table<Record7<String, Long, String, Boolean, LocalDateTime, LocalDateTime, String>> searchQueryTable = generateSearchQueryTable(ctx, userId, keyword);
        return ctx
    ...
    private static Table<Record7<String, Long, String, Boolean, LocalDateTime, LocalDateTime, String>> generateSearchQueryTable(DSLContext ctx, long userId, String keyword){
        String searchValue = "%" + keyword.replace("%", "\\%").replace("_", "\\_") + "%";
        return ctx.select(DSL.inline(SearchResultItemType.PROJECT.getName()).as("itemType"), PROJECT.ID, PROJECT.NAME, PROJECT.ARCHIVED, PROJECT.DATE_CREATED, PROJECT.LAST_ACTIVITY_DATE.as("lastActivity"), PROJECT.USER_NOTES)
                .from(PROJECT)
        ...
        .union(ctx.select(DSL.inline(SearchResultItemType.MODEL.getName()).as("itemType"), MODEL.ID, MODEL.NAME, MODEL.ARCHIVED, MODEL.DATE_CREATED, MODEL.LAST_ACTIVITY_DATE.as("lastActivity"), MODEL.USER_NOTES)
                .from(MODEL)
        ...
        .union(ctx.select(DSL.inline(SearchResultItemType.EXPERIMENT.getName()).as("itemType"), EXPERIMENT.ID, EXPERIMENT.NAME, EXPERIMENT.ARCHIVED, EXPERIMENT.DATE_CREATED, EXPERIMENT.LAST_ACTIVITY_DATE.as("lastActivity"), EXPERIMENT.USER_NOTES)
                .from(EXPERIMENT)
    ...
    private static SearchResult fetchIntoSearchResult(Record7<String, Long, String, Boolean, LocalDateTime, LocalDateTime, String> record) {
        return new SearchResult(SearchResultItemType.getEnumFromName(record.value1()), record.value2(), record.value3(), record.value4(), record.value5(), record.value6(), record.value7());
    }
}
