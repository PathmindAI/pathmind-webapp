...
public class SearchRepository {
    ...
    protected static List<SearchResult> findSearchResults(DSLContext ctx, Collection<SearchResultItemType> itemsTypes, long userId,
                                                          String keyword, int offset, int limit) {
        assert !itemsTypes.isEmpty();
        Table<Record9<String, Long, Boolean, LocalDateTime, LocalDateTime, String, String, String, String>> searchQueryTable = generateSearchQueryTable(ctx, itemsTypes, userId, keyword);
        return ctx
    ...
    protected static int countSearchResults(DSLContext ctx, Collection<SearchResultItemType> itemsTypes, long userId,
                                            String keyword) {
        assert !itemsTypes.isEmpty();
        return ctx.selectCount().from(generateSearchQueryTable(ctx, itemsTypes, userId, keyword)).fetchOne(DSL.count());
    }
    
    private static Table<Record9<String, Long, Boolean, LocalDateTime, LocalDateTime, String, String, String, String>> generateSearchQueryTable(DSLContext ctx, Collection<SearchResultItemType> itemsTypes, long userId, String keyword){
        assert !itemsTypes.isEmpty();
        ...
        if (itemsTypes.contains(SearchResultItemType.PROJECT)) {
        ...
        if (itemsTypes.contains(SearchResultItemType.MODEL)) {
        ...
        if (itemsTypes.contains(SearchResultItemType.EXPERIMENT)) {
