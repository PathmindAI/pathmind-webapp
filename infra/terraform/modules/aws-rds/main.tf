...
locals {
  value = "/${var.environment}/rds/${var.identifier}/DB_PASS"
}

# Generate a random pass and store in SSM
resource "null_resource" "db_password" {
  triggers = {
    param_group = module.db_parameter_group.this_db_parameter_group_arn
  }

  provisioner "local-exec" {
    command = "export DB_PASSWORD=$(head /dev/urandom | LC_ALL=C  tr -dc A-Za-z0-9 | head -c 16 ; echo '') && aws ssm put-parameter --value \"$DB_PASSWORD\" --name '${local.value}' --type SecureString --key-id alias/${var.environment}"
  }
}
